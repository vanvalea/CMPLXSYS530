...
to go  ;; forever button
  ...
  [ if who >= ticks [ stop ] ;; delay initial departure
    ifelse color = red
    [ look-for-food
     ;; kill it on the fifth tick
      if food-source-number = death_tick [
        if tick-count >= 5.0 [
          let die-prob random-float 1
          if die-prob < 1.5 [ die ] ;; killlllll
        ]
      ]
...
"food-in-pile4" 1.0 0 -955883 true "" "plotxy ticks sum [food] of patches with [pcolor = yellow]"
